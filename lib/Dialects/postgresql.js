...
exports.hasCollection = function (driver, name, cb) {
	...
	driver.execQuery(q, [ driver.config.schema, name ], function (err, rows) {
...
exports.getCollectionProperties = function (driver, name, cb) {
	...
	driver.execQuery(q, [ driver.config.schema, name ], function (err, cols) {
		if (err) return cb(err);
...
exports.addPrimaryKey = function(driver, tableName, columnName, cb){
	...
	return driver.execQuery(sql, [driver.config.schema, tableName, tableName + "_" + columnName + "_pk", columnName], cb);
};

exports.dropPrimaryKey = function(driver, tableName, columnName, cb){
	var sql = "ALTER TABLE ??.?? DROP CONSTRAINT ??;"
	return driver.execQuery(sql, [driver.config.schema, tableName, tableName + "_" + columnName + "_pk"], cb);
};

exports.addForeignKey = function(driver, tableName, options, cb){
	var sql = "ALTER TABLE ??.?? ADD FOREIGN KEY(??) REFERENCES ?? (??);"
	return driver.execQuery(sql, [driver.config.schema, tableName, options.name, options.references.table, options.references.column], cb);
};

exports.dropForeignKey = function(driver, tableName, columnName, cb){
	var sql = "ALTER TABLE ??.?? DROP CONSTRAINT ??;";
	return driver.execQuery(sql, [driver.config.schema, tableName, tableName + '_' + columnName + '_fkey'], cb);
};
...
exports.addCollectionColumn = function (driver, name, column, afterColumn, cb) {
	...
	return driver.execQuery(sql, [driver.config.schema, name] , cb);
};
...
exports.modifyCollectionColumn = function (driver, name, column, cb) {
	...
	if (driver.config.schema) {
		name = driver.config.schema + "." + name;
	}
